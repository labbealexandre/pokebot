from desmume.emulator import DeSmuME, SCREEN_PIXEL_SIZE, SCREEN_WIDTH, SCREEN_HEIGHT  # type: ignore
from desmume.controls import Keys, keymask  # type: ignore
...
from pokebot.utils import (
    memoryview_to_screen,
    resize_screen,
    add_red_border,
    put_text,
    generate_white_screen,
)
...
class Simulation:
    def __init__(
        ...
        self.upper_img: Screen = generate_white_screen(SCREEN_WIDTH, SCREEN_HEIGHT)
        self.lower_img: Screen = generate_white_screen(SCREEN_WIDTH, SCREEN_HEIGHT)
        self.img: Screen = generate_white_screen(SCREEN_WIDTH, 2 * SCREEN_HEIGHT)
        self.emu = DeSmuME()
        self.emu.open(rom)
        self.stop = False
        self.pause = False
    ...
    def get_key(self):
        ...
        if input_key in [ord(key) for key in KEYS_DICT]:
            ...
            t0 = int(time() * 1000)
            cv2.imwrite(f"pictures/upper_{t0}.jpg", self.upper_img)
            cv2.imwrite(f"pictures/lower_{t0}.jpg", self.lower_img)
    ...
    def compute_img(self):
        ...
        self.upper_img = memoryview_to_screen(
            gpu_framebuffer[: SCREEN_PIXEL_SIZE * 4], SCREEN_HEIGHT, SCREEN_WIDTH
        )
        self.lower_img = memoryview_to_screen(
            gpu_framebuffer[SCREEN_PIXEL_SIZE * 4 :], SCREEN_HEIGHT, SCREEN_WIDTH
        )

        self.img = concatenate((self.upper_img, self.lower_img), axis=0)
        self.img = resize_screen(self.img, self.resize_factor)
    ...
    def run(self):
        while True:
            ...
            if self.pause:
                cv2.imshow("image", self.img)  # type: ignore
                continue
